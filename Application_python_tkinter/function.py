...
def on_submit(email_entry, password_entry):
    ...
    name=iagi.curseur.execute('SELECT firstname FROM students WHERE email=?', (email,)).fetchone()

    if not check_empty_fields(email, mot_de_passe):
        return 0
    if verifier_utilisateur(email, mot_de_passe):
        messagebox.showinfo("Connexion réussie", "Bienvenue, " + name[0])
        return 1
    else:
...
def open_subject(title, image_name, pdf_files, tk_image5):
    ...
    Button(tk_subject, image=chap1, bd=0, activebackground="#fff", bg="#fff",
           command=lambda: ouvrir_pdf_avec_chrome(pdf_files[0])).place(x=75, y=180)
    chap2 = resize_image('chap2.jpg', (340, 40))
    Button(tk_subject, image=chap2, bd=0, activebackground="#fff", bg="#fff",
           command=lambda: ouvrir_pdf_avec_chrome(pdf_files[1])).place(x=85, y=260)
    chap3 = resize_image('chap3.jpg', (320, 40))
    Button(tk_subject, image=chap3, bd=0, activebackground="#fff", bg="#fff",
           command=lambda: ouvrir_pdf_avec_chrome(pdf_files[2])).place(x=80, y=340)

    # button pour la barre
    bar_image = resize_image('barre.jpg', (65, 65))
    Button(subject, image=bar_image, bd=0, bg="#fff", activebackground="#fff",
           command=lambda: barre(nav, tk_image3, closebtn, F1, F2, F3)).place(x=0, y=440)
    ...
    Button(nav, text="les modules", font=('Microsoft YaHei UI Light', 15, 'bold'), bd=0,
           bg="#042F69", fg="#fff", activebackground="#042F69", activeforeground="gray17",
           command=lambda: module).place(x=25, y=100)
    ...
    closebtn = Button(subject, image=close_image, activebackground="#042F69", bd=0, bg="#042F69",
                      command=lambda: barre(nav, tk_image3, closebtn, F1, F2, F3))
...
def oop(tk_image5):
    open_subject("python", 'python.jpg',
                 ["Chapitre 5 - Héritage (1).pdf", "Chapitre 6 - Gestion des fichiers.pdf",
                  "Base de données chap8.pdf"], tk_image5)


def ro(tk_image5):
    open_subject("R.O", 'RO_ar.jpg', ["01-Introduction_RO.pdf", "02-Prog Lineaire - ROPart1.pdf",
                                             "02-Prog Lineaire - ROPart2.pdf"], tk_image5)


def sdd(tk_image5):
    open_subject("SDD", 'sdd_ar.jpg', ["Structure de données en C - Seance 0 - complexite.pdf",
                                              "Structure de données en C - Seance 1 - Listes chainées .pdf",
                                              "Structure de données en C - Seance 2 - Listes chainées .pdf"],
                 tk_image5)


def linux(tk_image5):
    open_subject("Linux", 'linx_ar.jpg',
                 ["Systèmes d_exploitation - chap 1 - Introduction UNIX- LINUX.pdf",
                  "Systèmes d_exploitation - chap 2 - la gestion de fichier sous linux.pdf",
                  "Systèmes d_exploitation - chap 3 - gestion des utilisateurs et des groupes.pdf"], tk_image5)
...
def module(tk_image3):
    ...
    Button(modules, image=python_image, bd=0, bg="#fff", activebackground="#fff", command=lambda: oop(tk_image5)).place(
        x=110, y=200)

    ro_image = resize_image('RO.jpg', (320, 40))
    Button(modules, image=ro_image, bd=0, bg="#fff", activebackground="#fff", command=lambda: ro(tk_image5)).place(
        x=110, y=270)

    linux_image = resize_image('linux.jpg', (250, 40))
    Button(modules, image=linux_image, bd=0, bg="#fff", activebackground="#fff",
           command=lambda: linux(tk_image5)).place(x=110, y=340)

    sdd_image = resize_image('sdd.jpg', (320, 40))
    Button(modules, image=sdd_image, bd=0, bg="#fff", activebackground="#fff", command=lambda: sdd(tk_image5)).place(
        x=110, y=410)

    # button pour la barre
    bar_image = resize_image('barre.jpg', (65, 65))
    Button(modules, image=bar_image, bd=0, bg="#fff", activebackground="#fff",
           command=lambda: barre(nav, tk_image3, closebtn, F1, F2, F3)).place(x=0, y=440)
    ...
    global closebtn
    close_image = resize_image('close_bar.jpg', (65, 65))
    closebtn = Button(modules, image=close_image, activebackground="#042F69", bd=0, bg="#042F69",
                      command=lambda: barre(nav, tk_image3, closebtn, F1, F2, F3))
...
def login(root):
    ...
    Button(applog, image=next_button_image, activebackground="#fff", bd=0, bg="#fff",
           command=lambda: log_next(email_entry, password_entry, tk_image3)).place(x=350, y=380)
...
def create(firstname, lastname, email, password):
    ...
    if not is_valid_password(password):
        messagebox.showerror('Erreur', 'Le mot de pass doit compris entre 4 et 8 caractere.')
        return False
    ...
    if execute_sql(sql, data):
        messagebox.showinfo('message', 'Votre compte a été créé!')
        return True
    return False
...
def signup(root):
    ...
    Button(frame1, image=create_button_image, bd=0, activebackground="#fff", bg="#fff",
           command=lambda: sign_next(firstname, lastname, email, password, tk_image3)).place(x=60, y=294)
